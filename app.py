...
.rec-name{font-weight:900;color:white;font-size:15px;line-height:1.35;max-width:100%;overflow-wrap:anywhere;word-break:break-word;}.rec-meta{font-size:12px;color:#a9c8ff;margin-top:6px;line-height:1.45;max-width:100%;overflow-wrap:anywhere;word-break:break-word;}
.rec-card{align-items:flex-start !important;}
.rec-text{min-width:0;flex:1;}
.bundle-selected{color:#1d4ed8;font-weight:950;}
.bundle-plus{color:#475569;font-weight:900;margin:0 4px;}
.bundle-rec{color:#0f172a;font-weight:950;}
...
.logo-main,.hero-title,.overview-hero-title{color:#0f172a !important;text-shadow:none !important;}
.logo-sub,.hero-sub,.small-muted,.overview-hero-sub{color:#475569 !important;}
.neon-badge{
...
.metric-card,.chart-card,.kpi-grid-card,.highlight-card,.info-tile,.rec-card{
  background:#ffffff !important;
  border:1px solid #e5e7eb !important;
  box-shadow:0 10px 30px rgba(15,23,42,.08) !important;
}
.metric-title,.kpi-label,.highlight-label,.info-tile-label{color:#64748b !important;}
.metric-value,.chart-title,.kpi-number,.highlight-main,.info-tile-value,.rec-name,h1,h2,h3{color:#0f172a !important;text-shadow:none !important;}
.metric-sub,.highlight-sub{color:#2563eb !important;}
.metric-icon,.kpi-icon,.rec-rank,.stButton>button{
  background:#2563eb !important;
  color:#ffffff !important;
  box-shadow:none !important;
}
.rec-meta{color:#64748b !important;}
.conf-pill{
...
/* Make uploader bright and readable */
...
.stApp label, .stApp p, .stApp span, .stApp div, .stApp small{
  opacity:1 !important;
}
...
/* Keep text inside Plotly points/pie slices readable on white cards */
.js-plotly-plot .slicetext{
  fill:#0f172a !important;
  font-weight:800 !important;
}


/* ---------------- FORCE LIGHT MODE (ignore browser/system dark theme) ---------------- */
html, body, [data-testid="stAppViewContainer"], [data-testid="stAppViewBlockContainer"], .stApp{
  background:#ffffff !important;
  color:#0f172a !important;
  color-scheme: light !important;
}
[data-testid="stHeader"], header, .st-emotion-cache-18ni7ap, .st-emotion-cache-h4xjwg{
  background:#ffffff !important;
  color:#0f172a !important;
  box-shadow:none !important;
}
...
/* Make the hidden Streamlit top strip white too */
.stApp::before{
  content:"";
  position:fixed;
...
def chart_theme(fig, height=540):
    fig.update_layout(
        ...
        margin=dict(l=35, r=35, t=75, b=65),
    ...
    for tr in fig.data:
        ...
        # Keep trace names visible in chart legends (do not blank them).
...
def normalize_rule_item_text(value):
    """Convert FP-Growth frozenset/string cells into clean comma-separated product names."""
    if pd.isna(value):
        return ""
    ...
    # Handle strings like: frozenset({'item a', 'item b'})
    if text.startswith("frozenset("):
        try:
            parsed = ast.literal_eval(text.replace("frozenset", "set", 1))
            if isinstance(parsed, (set, frozenset, list, tuple)):
                return ", ".join(sorted(str(x).strip() for x in parsed if str(x).strip()))
        except Exception:
            pass

    # Remove common wrappers if literal parsing fails.
...
def split_rule_items(value):
    text = normalize_rule_item_text(value)
    if not text:
        return []
    return [x.strip() for x in text.split(",") if x.strip()]


def resolve_rules_path(path="mba_rules_dashboard.csv"):
    """Find the MBA rules CSV beside the app, in the working directory, or in /mnt/data."""
    candidates = []
    script_dir = os.path.dirname(os.path.abspath(__file__)) if "__file__" in globals() else os.getcwd()
    for base in [os.getcwd(), script_dir, "/mnt/data"]:
        candidates.append(os.path.join(base, path))
        candidates.extend(glob.glob(os.path.join(base, "mba_rules_dashboard*.csv")))
...
def load_rules(path="mba_rules_dashboard.csv"):
    ...
    rules = clean_columns(pd.read_csv(rules_path, low_memory=False))

    # Prefer the already-clean text columns exported by the notebook.
    rename_map = {}
    if "antecedents_str" in rules.columns:
        rename_map["antecedents_str"] = "antecedent"
    elif "antecedents" in rules.columns:
        rename_map["antecedents"] = "antecedent"
    elif "selected_product" in rules.columns:
        rename_map["selected_product"] = "antecedent"

    if "consequents_str" in rules.columns:
        rename_map["consequents_str"] = "consequent"
    elif "consequents" in rules.columns:
        rename_map["consequents"] = "consequent"
    elif "recommended_product" in rules.columns:
        rename_map["recommended_product"] = "consequent"

    rules = rules.rename(columns=rename_map)
    ...
    keep = req + [c for c in optional_cols if c in rules.columns]
    rules = rules[keep].copy()

    rules["antecedent"] = rules["antecedent"].apply(normalize_rule_item_text).str.lower().str.strip()
    rules["consequent"] = rules["consequent"].apply(normalize_rule_item_text).str.lower().str.strip()

    for c in ["support", "confidence", "lift", "rule_length"]:
        if c in rules.columns:
            rules[c] = pd.to_numeric(rules[c], errors="coerce")

    if "branch" in rules.columns:
        rules["branch"] = rules["branch"].astype(str).str.strip()

    # If rule_length is missing, calculate it from antecedent + consequent product counts.
    if "rule_length" not in rules.columns:
        rules["rule_length"] = (
            rules["antecedent"].apply(lambda x: len(split_rule_items(x))) +
            rules["consequent"].apply(lambda x: len(split_rule_items(x)))
        )

    rules = rules.dropna(subset=req)
    rules = rules[(rules["antecedent"] != "") & (rules["consequent"] != "")]

    # Keep only the bundle range used in the notebook: minimum 2 products and maximum 5 products.
    rules = rules[(rules["rule_length"] >= 2) & (rules["rule_length"] <= 5)]

    return rules, not rules.empty



def direct_recommendations(rules, selected_product, top_n=6, branch_filter=None):
    """Use the real support/confidence/lift values from mba_rules_dashboard.csv.

    The CSV can contain multi-product rules such as A, B -> C, D.
    For a selected product, the dashboard keeps the whole bundle together:
    selected product + the remaining products in that rule.
    """
    if rules.empty or not selected_product:
        return pd.DataFrame()

    selected_clean = str(selected_product).lower().strip()
    df = rules.copy()
    ...
    for _, r in df.iterrows():
        antecedent_items = split_rule_items(r.get("antecedent", ""))
        consequent_items = split_rule_items(r.get("consequent", ""))

        all_items_original = antecedent_items + consequent_items
        all_items_clean = [x.lower().strip() for x in all_items_original]

        if selected_clean not in all_items_clean:
            continue

        # Keep the complete bundle from the rule, excluding the selected product itself.
        bundle_items = []
        seen = set()
        for item in all_items_original:
            item_clean = item.lower().strip()
            if item_clean == selected_clean or item_clean in seen:
                continue
        ...
        rows.append({
            "Recommended Product": " + ".join(bundle_items),
            "support": float(r["support"]),
            "confidence": float(r["confidence"]),
            "lift": float(r["lift"]),
            "Rule": f"{r['antecedent']} → {r['consequent']}",
            "Bundle Size": int(r["rule_length"]) if "rule_length" in r and pd.notna(r["rule_length"]) else len(bundle_items) + 1,
            "branch": r.get("branch", "")
        })
    ...
    # Avoid repeated same bundle for the same product while keeping the strongest rule.
    return (
        out.drop_duplicates("Recommended Product")
...
def rules_product_options(rules, branch_filter=None):
    """Return selectable products only from the MBA rules CSV."""
    ...
    for col in ["antecedent", "consequent"]:
        for val in df[col].dropna().astype(str):
            for item in split_rule_items(val):
                item = item.strip()
                if item:
                    products.add(item)
...
def format_support_value(x):
    try:
        x = float(x)
    except Exception:
        return "0%"
    # MBA support is usually a proportion. Showing it as a percent avoids 0.000 for small but real values.
    if 0 <= x <= 1:
        return f"{x:.2%}"
    return f"{x:.4f}"


def format_confidence_value(x):
    try:
        return f"{float(x):.0%}"
    except Exception:
        return "0%"
...
def html_escape_text(x):
    import html
    return html.escape(str(x))


def live_mba(df, selected_product, top_n=6):
    """Live Market Basket Analysis using FP-Growth.

    Each invoice_id is treated as one transaction/basket. Products are encoded
    as a binary basket matrix, frequent itemsets are mined with FP-Growth, and
    association rules are generated with support, confidence, and lift.
    """
    ...
    try:
        ...
        # The app should not crash if mlxtend is missing. Install with:
        # pip install mlxtend
        return pd.DataFrame()

    work = df[["invoice_id", "product"]].copy()
    work["invoice_id"] = work["invoice_id"].astype(str).str.strip()
    work["product"] = work["product"].astype(str).str.lower().str.strip()
    selected_clean = str(selected_product).lower().strip()
    work = work[(work["invoice_id"] != "") & (work["product"] != "")]
    work = work.drop_duplicates(subset=["invoice_id", "product"])

    if work.empty or selected_clean not in set(work["product"]):
        return pd.DataFrame()

    total_transactions = work["invoice_id"].nunique()
    if total_transactions < 2:
        return pd.DataFrame()

    # Binary basket matrix: rows = invoices, columns = products, values = True/False.
    basket = pd.crosstab(work["invoice_id"], work["product"]).astype(bool)

    # Use a practical minimum support. Start with at least two transactions where possible;
    # if too few rules are found, relax to one transaction support.
    support_candidates = []
    if total_transactions >= 2:
        support_candidates.append(max(2 / total_transactions, 0.001))
    support_candidates.append(max(1 / total_transactions, 0.001))
    ...
    for min_sup in support_candidates:
        try:
            ...
            if frequent_itemsets.empty:
                continue
            rules_fp = association_rules(frequent_itemsets, metric="confidence", min_threshold=0.0)
            if rules_fp.empty:
                continue
            # Keep only rules where selected product is in antecedent or consequent.
            mask = rules_fp["antecedents"].apply(lambda s: selected_clean in {str(x).lower().strip() for x in s}) | \
                   rules_fp["consequents"].apply(lambda s: selected_clean in {str(x).lower().strip() for x in s})
            rules_fp = rules_fp[mask].copy()
            if not rules_fp.empty:
                final_rules = rules_fp
                used_support = min_sup
                break
        except Exception:
            continue
    ...
    for _, r in final_rules.iterrows():
        antecedents = [str(x).lower().strip() for x in r["antecedents"]]
        consequents = [str(x).lower().strip() for x in r["consequents"]]

        if selected_clean in antecedents:
            rec_items = [x for x in consequents if x != selected_clean]
        elif selected_clean in consequents:
            rec_items = [x for x in antecedents if x != selected_clean]
        else:
            continue
    ...
    out = out.drop_duplicates("Recommended Product").head(top_n)
    return out

def popular_fallback(df, selected_product, top_n=6):
    """Last-resort fallback. Calculates an estimated lift instead of forcing lift = 1.00."""
    pop = (
    ...
    consequent_support = np.maximum(support, 1 / total_inv)
    estimated_lift = confidence / consequent_support
...
def clean_product_label(name, max_len=42):
    """Shorten product names for cleaner bundle network labels."""
    s = str(name).lower().strip()
    remove_patterns = [
        r"\s*-\s*other items\s*-\s*general\s*$",
        r"\s*-\s*general\s*$",
        r"\s*-\s*duplicate item.*$",
        r"\s*-\s*damaged.*$",
        r"\s*-\s*sample.*$",
    ]
...
def build_bundle_network_figure(rec_df, selected_product, product_summary=None, height=620):
    """Build a Plotly-only network diagram for product bundling; no networkx needed."""
    if rec_df is None or rec_df.empty:
        return go.Figure()

    edge_rows = []
    for _, r in rec_df.iterrows():
        target = str(r.get("Recommended Product", "")).strip()
        if not target:
            continue
        edge_rows.append({
            "source": str(selected_product),
            "target": target,
            "confidence": float(r.get("confidence", 0) or 0),
            "lift": float(r.get("lift", 1) or 1),
            "support": float(r.get("support", 0) or 0),
        })
    if not edge_rows:
        return go.Figure()

    # Add additional links among recommended products when they appear in the rule text.
    # This creates linkage like a real association network instead of a bar chart.
    for _, r in rec_df.iterrows():
        rule = str(r.get("Rule", ""))
        if "→" in rule:
            left, right = [x.strip() for x in rule.split("→", 1)]
            if left and right and left != selected_product:
                edge_rows.append({
                    "source": left,
                    "target": right,
                    "confidence": float(r.get("confidence", 0) or 0),
    ...
    # ABC mapping for node color and size
    abc_map = {}
    if product_summary is not None and not product_summary.empty and "product" in product_summary.columns:
        tmp = product_summary.copy()
        tmp["product_key"] = tmp["product"].astype(str).str.lower().str.strip()
        abc_map = dict(zip(tmp["product_key"], tmp.get("ABC", "C")))

    node_strength = {n: 0.0 for n in nodes}
    for e in edge_rows:
        node_strength[e["source"]] += e["lift"]
        node_strength[e["target"]] += e["lift"]

    # Deterministic circular layout with selected product in the center
    import math
    pos = {str(selected_product): (0.0, 0.0)}
    others = [n for n in nodes if n != str(selected_product)]
    radius = 1.0
    for i, n in enumerate(others):
        ang = 2 * math.pi * i / max(len(others), 1)
        pos[n] = (radius * math.cos(ang), radius * math.sin(ang))

    node_color_map = {"A": "#ef3340", "B": "#f59e0b", "C": "#2a9d8f"}
    node_size_map = {"A": 34, "B": 28, "C": 23}

    fig = go.Figure()

    # Edges: thickness = confidence; color = lift
    lifts = [e["lift"] for e in edge_rows]
    ...
    for e in edge_rows:
        ...
        width = max(2, min(10, 2 + e["confidence"] * 10))
        fig.add_trace(go.Scatter(
            x=[x0, x1], y=[y0, y1],
            mode="lines",
            line=dict(color="rgba(15,23,42,.85)", width=width),
            hovertemplate=(
                f"<b>{clean_product_label(e['source'], 60)}</b> → <b>{clean_product_label(e['target'], 60)}</b><br>"
                f"Confidence: {e['confidence']:.0%}<br>Lift: {e['lift']:.2f}<br>Support: {format_support_value(e['support'])}<extra></extra>"
            ),
            showlegend=False
        ))

    # Invisible marker just to display Lift color scale on the right
    fig.add_trace(go.Scatter(
        ...
        marker=dict(
            ...
            color=[max_lift],
            size=1,
            colorbar=dict(title=dict(text="Lift", font=dict(size=14, color="#000000")), tickfont=dict(size=12, color="#000000"), thickness=16, len=.72)
        ),
        hoverinfo="skip",
        showlegend=False
    ))

    # Nodes by ABC class to show legend
    for abc_class, label in [("A", "A (High Value)"), ("B", "B (Moderate Value)"), ("C", "C (Low Value)")]:
        xs, ys, texts, sizes, hovers = [], [], [], [], []
        for n in nodes:
            cls = abc_map.get(str(n).lower().strip(), "A" if n == str(selected_product) else "C")
            if cls != abc_class:
                continue
            x, y = pos.get(n, (0, 0))
            xs.append(x); ys.append(y)
            texts.append(clean_product_label(n, 30))
            sizes.append(node_size_map.get(cls, 24) + min(14, node_strength.get(n, 0) * 1.2))
            hovers.append(f"<b>{clean_product_label(n, 80)}</b><br>Class: {label}<br>Node size: association strength<extra></extra>")
        if xs:
            fig.add_trace(go.Scatter(
                x=xs, y=ys,
                mode="markers+text",
                ...
                marker=dict(
                    size=sizes,
                    color=node_color_map.get(abc_class, "#2a9d8f"),
    ...
    fig.update_layout(
        title=dict(text="Product Bundling (Association Rules Network)", font=dict(size=20, color="#000000")),
        template="plotly_white",
        height=height,
        paper_bgcolor="rgba(0,0,0,0)",
        plot_bgcolor="rgba(0,0,0,0)",
        margin=dict(l=90, r=90, t=60, b=80),
        showlegend=True,
        legend=dict(
            x=0.02,
            y=0.98,
    ...
    fig.add_annotation(
        ...
        align="left",
        text=("Node color: ABC value class<br>"
              "Node size: association strength<br>"
              "Edge width: confidence<br>"
              "Color bar: lift"),
        font=dict(size=13, color="#000000"),
        bgcolor="rgba(248,250,252,.95)",
        bordercolor="#e5e7eb",
        borderwidth=1,
        borderpad=8,
    )
...
st.sidebar.caption("Forecast loaded automatically")
...
with left:
    ...
    st.markdown("<div class='hero-sub'></div>", unsafe_allow_html=True)
with right:
    st.markdown(f"""
...
if page == "Overview":
    ...
    with oc1:
        ...
        fig.update_layout(xaxis_title="Date", yaxis_title="Sales (Million Nu.)", hovermode="x unified")
    ...
    with oc2:
        ...
        fig = go.Figure(data=[go.Pie(
            ...
            insidetextfont=dict(color="#0f172a", size=13),
        ...
        fig.add_annotation(
            ...
            x=.5, y=.5, showarrow=False, font=dict(size=17, color="#0f172a")
        )
        st.plotly_chart(chart_theme(fig, 560), use_container_width=True, config=CHART_CONFIG)
        section_end()
    ...
    if sales_view == "Overall weekly trends":
        ...
        fig.update_layout(xaxis_title="Week", yaxis_title="Sales (Million Nu.)", hovermode="x unified")
        fig.update_yaxes(tickprefix="Nu. ")
        st.plotly_chart(chart_theme(fig, 560), use_container_width=True, config=CHART_CONFIG)
    ...
    section_start("Product Bundling Network")
    rules, rules_found = load_rules()

    if rules_found:
        st.caption("Using the uploaded MBA rules CSV. Product dropdown is built from all products in mba_rules_dashboard.csv, and support, confidence, and lift are taken directly from that file.")
    else:
        st.caption("No MBA rules CSV was found, so the dashboard will calculate live MBA using FP-Growth from the uploaded sales data.")

    # Since the new MBA rules CSV includes all products, use the CSV as the main product source.
    # This prevents missing products and avoids fake fallback lift = 1.
    if rules_found:
        available = rules_product_options(rules, selected_branches)
    else:
        available = sorted(filtered_mba["product"].dropna().astype(str).unique()) if not filtered_mba.empty else []

    if not available:
        st.warning("No products are available for the selected filters. If branch filters are active, that branch may not have matching rules in mba_rules_dashboard.csv.")
        section_end()
        st.stop()
    ...
    with c2:
        top_n = st.slider("Number of bundle recommendations", 2, 5, 3)

    if rules_found:
        rec = direct_recommendations(rules, selected_prod, top_n, selected_branches)
        source = "mba_rules_dashboard.csv"
        if rec.empty:
            st.warning("This product is listed in the MBA rules file, but no rule matched the current branch filter. Try clearing branch filters or selecting another product.")
    else:
        rec = live_mba(filtered_mba, selected_prod, top_n)
        source = "live FP-Growth MBA from uploaded sales data"
        if rec.empty:
            st.warning("No live FP-Growth rule could be created for this product from the uploaded sales data.")

    if rec.empty:
        st.info("No product bundle can be created for this selection.")
    else:
        st.caption(f"Recommendation source: {source}")
        for i, row in rec.reset_index(drop=True).iterrows():
            selected_html = html_escape_text(clean_product_label(selected_prod, 70))
            rec_html = html_escape_text(clean_product_label(row['Recommended Product'], 120))
            rule_html = html_escape_text(clean_product_label(row.get('Rule',''), 170))
            support_txt = format_support_value(row['support'])
            lift_txt = format_lift_value(row['lift'])
            conf_txt = format_confidence_value(row['confidence'])
            bundle_size_txt = int(row.get("Bundle Size", 0)) if "Bundle Size" in rec.columns and pd.notna(row.get("Bundle Size", None)) else ""
            bundle_meta = f"Bundle size: {bundle_size_txt} products | " if bundle_size_txt else ""
            st.markdown(f"""
            <div class='rec-card'>
              <div class='rec-rank'>{i+1}</div>
              <div class='rec-text'>
                <div class='rec-name'><span class='bundle-selected'>{selected_html}</span><span class='bundle-plus'> + </span><span class='bundle-rec'>{rec_html}</span></div>
                <div class='rec-meta'>{rule_html}<br>{bundle_meta}Support: {support_txt} | Lift: {lift_txt}</div>
              </div>
              <div class='conf-pill'>{conf_txt}</div>
            </div>
            """, unsafe_allow_html=True)

        product_summary = make_abc_ved_summary(filtered)
        fig = build_bundle_network_figure(rec.reset_index(drop=True), selected_prod, product_summary, height=700)

...
def build_bundle_network_figure(rec_df, selected_product, product_summary=None, height=620):
    ...
    for e in edge_rows:
        ...
        fig.add_trace(go.Scatter(
            ...
            hovertemplate=(
                ...
                f"Confidence: {e['confidence']:.0%}<br>Lift: {e['lift']:.2f}<br>Support: {e['support']:.3f}<extra></extra>"
            ),
            showlegend=False
        ))
    ...
    for abc_class, label in [("A", "A (High Value)"), ("B", "B (Moderate Value)"), ("C", "C (Low Value)")]:
        ...
        for n in nodes:
            ...
            texts.append(clean_product_label(n))
...
if page == "Overview":
    ...
    if rec.empty:
        ...
        fig = build_bundle_network_figure(rec.reset_index(drop=True), selected_prod, product_summary, height=650)
